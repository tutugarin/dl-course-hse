...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            gradt = np.add(grad, self.weight_decay * param)
            np.add(self.momentum * m, gradt, out=m)
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            gradt = grad + np.dot(self.weight_decay, param)
            np.add(self.beta1 * m, (1 - self.beta1) * gradt, out=m)
            np.add(self.beta2 * v, (1 - self.beta2) * gradt * gradt, out=v)
